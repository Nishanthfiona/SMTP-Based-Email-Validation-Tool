                ...
                for num in messages[0].split():
                    ...
                    if status == "OK":
                        ...
                        if (
                            ...
                            or "undelivered" in subject.lower()
                        ):
...
def process_emails(
    ...
    for idx, row in df_subset.iterrows():
        ...
        is_valid, validation_time = validate_email(
            email_address, gmail_user, gmail_app_password
        )
...
if input_excel:
    ...
    df = pd.read_excel(input_excel)
    input_filename = input_excel.name.split(".")[
        0
    ...
    email_column = st.selectbox(
        "Select the column with email addresses", options=df.columns
    )

    # Input for start and end rows
    start_row = st.number_input("Start Row", min_value=1, value=1)
    end_row = st.number_input(
        "End Row", min_value=start_row, value=min(start_row + 9, len(df))
    )
    ...
    if not st.session_state["invalid_emails"].empty:
        ...
        invalid_filename = generate_filename(
            input_filename, start_row, end_row, "invalid"
        )
